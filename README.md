...
### Web API and React frontend (new)
...
2. Run the backend API (model file `sign_language_interpreter_model.h5` must be present):
...
3. Start the React frontend (requires Node.js):
...
4. Open http://localhost:5173 (vite default) and allow camera access. Use the UI to capture frames or upload images; predictions are requested from the backend at http://localhost:8000/predict.
